...
class AppDelegate: FlutterAppDelegate {
  // A headless engine. In multi-window mode the Dart side creates every window
  // (including the main one) via the windowing APIs, so the macOS runner must
  // NOT attach a FlutterViewController up front — doing so would make
  // `enableMultiView` abort the first time Dart creates a window.
  var engine: FlutterEngine?

  override func applicationDidFinishLaunching(_ notification: Notification) {
    let engine = FlutterEngine(name: "panes", project: nil)
...
// MARK: - FFI bridge
//
// The dock is driven from Dart over FFI (see lib/panels/native_dock.dart):
// forward calls are the `@_cdecl` functions below; reverse drag/drop events go
// through the registered C function pointers. The C ABI is macos/Runner/panes_dock.h.
...
// MARK: - @_cdecl C ABI (resolved from Dart via DynamicLibrary.executable())
