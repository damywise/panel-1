...
//
// This file needs the windowing registry only to start a tear-off, so it opts
// in to the same analyzer ignores as the rest of the framework.
...
class PanelDock extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    void detach(String id) => manager.detach(id, registry);
    ...
    return Stack(
      children: <Widget>[
        ...
        if (manager.nativeHoverSide != null)
          Positioned.fill(child: _NativeDropZoneOverlay(manager: manager, activeSide: manager.nativeHoverSide!)),
      ],
    );
  }
}
    ...
    return Container(
      ...
      child: Row(
        children: <Widget>[
          ...
          if (cfg.allowDetach && activeId != null)
  ...
  Widget build(BuildContext context) {
    ...
    return Draggable<_TabDrag>(
      ...
      onDraggableCanceled: (_, _) {
        if (widget.manager.config.allowDetach) widget.onDetach(widget.descriptor.id);
        widget.manager.endDrag();
      },
