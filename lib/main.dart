...
Future<void> main() async {
  ...
  final PanelManager manager = PanelManager(config: PanelDockConfig(storage: JsonFileStorage()))
  ...
  runWidget(PanesApp(manager: manager));
}
...
class PanesApp extends StatefulWidget {
  const PanesApp({super.key, required this.manager});
...
class _WorkspaceState extends State<_Workspace> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      // Make the main window's native title bar transparent/full-size.
      NativeDock.instance.decorateMain();
      if (!mounted) return;
      ...
      if (auto.isNotEmpty) {
        m.detach(auto, WindowRegistry.of(context));
      }
